...
const field = (
  ...
  if (found === undefined) throw new Error(`Missing ${subcategoryId}/${fieldId}`);
  return found;
};
...
describe("permission settings category", () => {
  it("adds users and roles through one preset mentionable control", async () => {
    ...
    if (add.kind !== "mentionable-select") throw new Error("Expected mentionable select");
  ...
  it("renders explicit subject types and paginates preset controls", async () => {
    ...
    expect(rendered.location.pageCount).toBeGreaterThan(1);
  ...
  it("requires a second clear click within the confirmation window", async () => {
    const { category, setPresetSubjects, advanceTime } = setup([
      { subjectType: "role", subjectId: "role-1" },
    ]);
    ...
    expect(setPresetSubjects).not.toHaveBeenCalled();
    ...
    await clear.mutate(context);
    expect(setPresetSubjects).toHaveBeenCalledOnce();
  });
  ...
  it("paginates inspection controls and removes one selected rule", async () => {
    const rules = Array.from({ length: 80 }, (_, index) => makeRule(index));
