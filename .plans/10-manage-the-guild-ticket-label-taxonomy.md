...
- 2026-07-17: PR review also found that an unbounded name-and-description display could silently hide manageable labels at Discord's text limit. Resolution bounds active choices at 20, renders every exact active name within the component budget, keeps inactive labels as immutable history, and permits replacements after deactivation.
- 2026-07-17: A follow-up review found direct first-write store callers could bypass initialization and later exceed the active bound. Resolution shares one transaction-scoped initializer across explicit initialization and creation, including domain-error commits, so all public create paths begin with the five defaults and remain bounded.
