...
- Text commands require a registered command keyword after the prefix (for example, `;ping`); a bare prefix or unknown keyword is unconsumed. They default to `!`, disable on a trimmed empty prefix, accept prefixes of 1–8 Unicode code points, normalize command names to lowercase, preserve the untouched argument tail, ignore bot/webhook messages, and report whether a message was consumed.
- Text commands are an opt-in Protocord provider capability. Prod defaults the prefix to empty, which omits the privileged message intents and listener; an explicit configured prefix enables the private bot's ping validation surface.
...
- Prod refreshes Protocord's complete catalog globally and clears stale guild commands first; Discord's authoritative `set` operation updates changed slash/context commands and removes stale ones on every restart.
...
- Prod validation: 26/26 app tests pass, including all four ping surfaces, configured-prefix private-bot text dispatch, disabled text capability gating, authoritative global command refresh, and gateway wiring.
