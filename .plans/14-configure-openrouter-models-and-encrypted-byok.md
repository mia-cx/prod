...
- [x] Provide Discord credentials, an OpenRouter API key, and a generated 32-byte base64 encryption key through local secrets. Select OpenRouter, configure a model ID, configure and clear BYOK in Discord, restart Prod, and inspect logs/UI for accidental secret exposure. (The minimal authenticated model request and request-level deployment-fallback check are deferred to the inference-wiring slice — nothing consumes `resolve()` yet.)
- [x] Record the validation outcome without secrets, raw tokens, private ticket content, or unredacted diagnostics.
...
- 2026-07-22: HITL validation passed: Mia configured and cleared BYOK through the Discord Model settings page against a real deployment with no secret exposure observed. The authenticated OpenRouter request check is explicitly deferred to the future inference slice, where `resolve()` gains its first consumer and the guild-key log-redaction constraint must land with it.
